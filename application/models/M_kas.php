...
class M_kas extends CI_Model
{
	...
	public function getKas($params = [])
	{
		...
		if (!empty($params['order_by'])) {
			$this->db->order_by($params['order_by'], isset($params['order_dir']) ? $params['order_dir'] : 'ASC');
		}
	...
	public function getKasByWarga($idWarga)
	{
		return $this->db->where('idWarga', $idWarga)
			->where('jenis', 'masuk')
			->where('status', 'kas')
	...
	public function uploadBukti($idKas, $file)
	{
	...
	public function getPembayaranBulanan($idWarga, $tahun, $status)
	{
		$result = [];
		for ($i = 1; $i <= 12; $i++) {
